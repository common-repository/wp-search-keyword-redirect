...
<div class="wrap">
	...
	<div class="wrap ww-keyword-redirect">
		...
		<p><?php _e( 'This plugin Matches search queries to keywords. On a match it redirects to specific URLs.' ); ?>
			<br><?php _e( 'Note, this means any match for a non case-sensative string.' ); ?></p>

		<p><strong><?php _e( 'Example' ); ?>
				:</strong> <?php _e( 'The plugin will redirect with these sample search queries for the keyword <span class="hl">test</span>' ); ?>
			:</p>
		...
		<form method="post" action="">
			<div class="redirects">
				<table class="redirect_table">
					...
					<tr>
						<td>
							<small><?php _e( 'example' ); ?>: <?php _e( 'Keyword' ); ?> </small>
						</td>
						<td>
							<small><?php _e( 'example' ); ?>: <?php echo get_option( 'home' ); ?>/destination-url/
							</small>
						</td>
			...
			<script type="text/template" class="template" id="redirect">
				<tr>
					...
					<td><a href="#" class="delete_redirect button"
					       title="Delete Redirect"><?php _e( 'Delete Redirect' ); ?><a/></td>
				</tr>
			</script>
